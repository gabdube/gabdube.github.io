...
interface Link extends Element {
    dataset: { url?: string, url2?: string, goto?: string, }
}
...
interface CodeViewData {
    tabs: CodeViewTab[];
    current_tab: number;
}

const CODE_VIEW_DATA: CodeViewData = { 
    tabs: [],
...
function fetch_cache(target_url: string): CodeViewTab | null {
...
function refresh_tab_display() {
    ...
    const show_demo = document.getElementById("demoheaderShowDemo") as HTMLElement;
...
function show_tab(tab: CodeViewTab) {
...
async function add_tab(url: string): Promise<CodeViewTab> {
    async function generate_code_element(url: string): Promise<HTMLElement> {
    ...
    async function close_tab(tab: CodeViewTab) {
        if (CODE_VIEW_DATA.current_tab == tab.index) {
...
function scroll_to_code(code: string) {
    const codeview = document.getElementById("codeview") as HTMLElement;
    const code_element = codeview.firstChild?.firstChild as HTMLElement;
    if (!code_element) {
    ...
    if (found) {
        let previous = found.previousSibling as HTMLElement;
        if (!previous) { return; }
...
async function load_code(link: Link) {
...
export function init_codeview() {
    ...
    setTimeout(() => {
        ...
        for (const link of content.getElementsByClassName("code-link") as HTMLCollectionOf<Link>) {
