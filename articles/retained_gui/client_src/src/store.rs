...
impl StoreWriter {
    ...
    pub fn new() -> Self {
        StoreWriter {
            data: vec![0u8; 1024*64],    // 64kb should be more than enough
            data_offset: 0
        }
    }
    ...
    pub fn write<T: IntoBytes+Immutable>(&mut self, value: &T) {
        assert!(align_of::<T>() >= MIN_ALIGN, "Data alignment must be at least 4 bytes");
    ...
    pub fn write_str(&mut self, value: &str) {
        ...
        self.write(&[length as u32, padded_length as u32]);
        value.write_to_prefix(self.remaining_bytes()).unwrap();
...
impl<'a> StoreReader<'a> {
    ...
    pub fn try_read_bool(&mut self) -> Result<bool, Error> {
        let value: u32 = self.try_read()?;
        match value {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(assets_err!("Boolean value should be 0 or 1, got {}", value))
        }
    }
...
pub trait StoreLoad: Sized {
    fn store(&self, writer: &mut StoreWriter);
